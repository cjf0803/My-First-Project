    ...
    function updateUser(userid) {
      if(confirm("是否修改?")){
        $.ajax({
          ...
          success:function(result) {
            var input = "";
            input += "<p>用户id(默认不修改):<input type='text' name='userid' class='form-control'autocomplete='off' value='" + result.userid + "'></p>";
            input += "<p>用户名:<input type='text' name='uname' class='form-control' autocomplete='off' value='" + result.uname + "'></p>";
            input += "<p>用户密码:<input type='text' name='upwd' class='form-control' autocomplete='off' value='" + result.upwd + "'></p>";
            input += "<p>用户类型(1:admin;2:customer):<input type='text' name='type' class='form-control' autocomplete='off' value='" + result.type + "'></p>";
            input += "<p>用户年龄:<input type='text' name='age' class='form-control' autocomplete='off' value='" + result.age + "'></p>";
            input += "<p>用户余额:<input type='text' name='money' class='form-control' autocomplete='off' value='" + result.money + "'></p>";
            input += "<p>用户注册时间:<input type='date' name='time' class='form-control'  autocomplete='off' value='" + result.time + "'></p>";
            $("#for").html(input);
          }
...
<section class="container-fluid">
  ...
  <div class="row">
    ...
    <div class="col-sm-9 col-sm-offset-3 col-md-10 col-lg-10 col-md-offset-2 main" id="main">
      <ol class="breadcrumb">
        <form action="/user/findUserMo" method="post" class="navbar-form navbar-right" role="search">
          <div class="input-group">
            <input type="text" name="uname" class="form-control" autocomplete="off" placeholder="键入关键字搜索用户" maxlength="15">
          ...
          <table class="table table-striped table-hover">
            ...
            <tbody>
             ...
             <c:forEach items="${list}" var="p">
                ...
                <td>
                <button type="button" class="btn btn-primary" onclick="delete1(${p.userid})">删除</button><%--<a href="/person/deletePerson?id=${p.id}">删除</a>--%>

                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#myModal" onclick="updateUser(${p.userid})">修改</button>
                </td>
        ...
        <footer class="message_footer">
          <nav>
            ...
            <ul class="pagination pagenav">
              ...
              <li class="active"><a href="/user/findUser?index=1">首页</a></li>

              <li>
                <c:if test="${index>1}">
                <a href="/user/findUser?index=${index-1}">上页</a>
                </c:if>
              </li>

              <li>
                <c:if test="${index<totalPage}">
                  <a href="/user/findUser?index=${index+1}">下页</a>
                </c:if >
              </li>
              <li>
                <c:if test="${index!=totalPage}">
                  <a href="/user/findUser?index=${totalPage}">末页</a>
                </c:if>
              </li>
