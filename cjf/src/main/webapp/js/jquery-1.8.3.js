...
jQuery.Callbacks = function( options ) {
	...
	var // Last fire value (for non-forgettable lists)
		...
		fired,
		// Flag to know if lists is currently firing
		firing,
		...
		fire = function( data ) {
			...
			firingStart = 0;
			firingLength = lists.length;
			firing = true;
			for ( ; lists && firingIndex < firingLength; firingIndex++ ) {
				if ( lists[ firingIndex ].apply( data[ 0 ], data[ 1 ] ) === false && options.stopOnFalse ) {
		...
		self = {
			...
			// Control if a given callback is in the lists
			has: function( fn ) {
				return jQuery.inArray( fn, lists ) > -1;
			},
...
jQuery.extend({
	...
	Deferred: function( func ) {
		...
		jQuery.each( tuples, function( i, tuple ) {
			...
			// deferred[ resolve | reject | notify ] = lists.fire
			deferred[ tuple[0] ] = lists.fire;
		...
		if ( (value && typeof value === "string") || value === undefined ) {
			...
			for ( i = 0, l = this.length; i < l; i++ ) {
				...
				if ( elem.nodeType === 1 && elem.className ) {
					...
					// loop over each item in the removal lists
...
sortOrder = docElem.compareDocumentPosition ?
	...
	function( a, b ) {
		...
		// to build up a full lists of the parentNodes for comparison
		while ( cur ) {
			ap.unshift( cur );
			cur = cur.parentNode;
		}
...
function addToPrefiltersOrTransports( structure ) {
	...
	return function( dataTypeExpression, func ) {
		...
		var dataType, lists, placeBefore,
		...
		if ( jQuery.isFunction( func ) ) {
			...
			for ( ; i < length; i++ ) {
				...
				lists = structure[ dataType ] = structure[ dataType ] || [];
				// then we add to the structure accordingly
				lists[ placeBefore ? "unshift" : "push" ]( func );
			}
		}
	};
}
...
function inspectPrefiltersOrTransports( structure, options, originalOptions, jqXHR,
	...
	var selection,
		lists = structure[ dataType ],
		i = 0,
		length = lists ? lists.length : 0,
		executeOnly = ( structure === prefilters );

	for ( ; i < length && ( executeOnly || !selection ); i++ ) {
		selection = lists[ i ]( options, originalOptions, jqXHR );
